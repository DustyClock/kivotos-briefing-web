(() => {
  ...
  // GitHub Device Flow용 Cloudflare Worker
  const AUTH_PROXY =
    "https://kivotos-github-auth.dydghdus12.workers.dev";
  ...
  async function startDeviceFlow() {
    if (!config.githubAppClientId) {
      $("#login-help").textContent =
        "GitHub App Client ID가 아직 설정되지 않았습니다. 먼저 화면 미리보기를 확인할 수 있습니다.";
      return;
    }

    try {
      $("#login-help").textContent =
        "GitHub 인증 코드를 요청하고 있습니다...";

      /*
       * GitHub에 브라우저에서 직접 요청하면 CORS에 막히므로
       * Cloudflare Worker를 통해 Device Code를 발급받는다.
       */
      const response = await fetch(
        `${AUTH_PROXY}/device/code`,
        {
          method: "POST",
          headers: {
      ...
      $("#login-help").textContent =
        error?.message || "GitHub 인증을 시작하지 못했습니다.";
    }
  }

  async function pollForToken(device) {
    let interval =
      Number(device.interval || 5) * 1000;

    const deadline =
      Date.now() +
      Number(device.expires_in || 900) * 1000;

    while (Date.now() < deadline) {
      await wait(interval);

      /*
       * Access Token 요청 역시 GitHub에 직접 보내지 않고
       * Cloudflare Worker를 거친다.
       */
      const response = await fetch(
        `${AUTH_PROXY}/access-token`,
        {
          method: "POST",
          headers: {
            Accept: "application/json",
      ...
      if (!response.ok) {
        throw new Error(
          `GitHub 토큰 요청 실패 (${response.status})`
        );
      }
  ...
  async function verifyIdentity() {
    ...
    if (
      Number(config.allowedGithubUserId) &&
      Number(user.id) !==
        Number(config.allowedGithubUserId)
    ) {
      state.token = null;
  ...
  async function loadDashboard() {
    try {
      const [latest, status] =
        await Promise.all([
          readData("latest.json"),
          readData("status.json")
        ]);
  ...
  async function readData(filename) {
    ...
    const url =
      ...
      `${config.githubOwner}/` +
      `${config.privateRepository}/contents/` +
      `${config.privateDataPath || "dashboard-data"}/` +
      `${filename}`;
  ...
  function renderAll(latest) {
    $("#new-count").textContent =
      String(latest.items?.length || 0);

    $("#last-collected").textContent =
      `마지막 갱신 ${formatDate(
        latest.generated_at ||
    ...
    $("#news-list").innerHTML =
      ...
      <article class="news-card">
        ...
        <div>
          ...
          <div class="badges">
            <span class="badge ${item.importance}">
            ...
            ${
              item.is_deleted
                ? '<span class="badge urgent">원문 삭제</span>'
                : ""
            }
          </div>
        </div>
        ...
        .join("") ||
      '<p class="muted">조건에 맞는 소식이 없습니다.</p>';
  ...
  function renderMonths(months) {
    $("#month-list").innerHTML =
      months
        ...
        .join("") ||
      '<p class="muted">아직 지난 기록이 없습니다.</p>';
  }
  ...
  function renderSchedules() {
    ...
    $$("[data-delete-time]").forEach(
      (button) =>
        button.addEventListener(
          "click",
          () =>
            deleteSchedule(
              button.dataset.deleteTime
            )
        )
    );

    $("#next-run").textContent =
      state.schedules[0] || "없음";
  }
  ...
  function renderStatus() {
              ...
              <span class="status-pill">
                ${
                  source.consecutive_failures
                    ? `실패 ${source.consecutive_failures}회`
                    : "정상"
                }
              </span>
            </div>`
    ...
    $("#service-health").textContent =
      failures ? "일부 오류" : "정상";

    $("#health-detail").textContent =
      `${sources.length || 7}개 출처`;
  ...
  async function resetSchedules() {
    if (
      !await confirmAction(
        "전체 초기화",
        "예약 시간을 기본값 09:00, 21:00으로 초기화할까요?"
      )
    ) {
      return;
    }
      ...
      items: [
        {
          ...
          first_seen_at: now,
          url:
            "https://forum.nexon.com/bluearchive/",
          tags: [
            "maintenance"
          ]
        },

        {
          ...
          first_seen_at: now,
          url:
            "https://www.youtube.com/@BlueArchive_JP",
          tags: [
            "video"
          ],
          thumbnail_url:
            "https://i.ytimg.com/vi/aqmDeMZFwt8/hqdefault.jpg"
        },

        {
          ...
          title_ko:
            "신규 이벤트 안내",
          summary_ko:
            "글로벌 서버 신규 이벤트 일정 안내입니다.",
          published_at: now,
          first_seen_at: now,
          url:
            "https://forum.nexon.com/bluearchive-en/",
          tags: [
            "event"
          ]
        }
